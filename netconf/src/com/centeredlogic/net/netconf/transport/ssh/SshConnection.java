 ...
 */
package com.centeredlogic.net.netconf.transport.ssh;
...
import com.centeredlogic.log.LoggerIf;
import com.centeredlogic.net.netconf.Capabilities;
import com.centeredlogic.net.netconf.transport.HelloResponseProcessorIf;
 ...
 * @since   1.6
 */
abstract class SshConnection extends com.centeredlogic.net.ssh.SshConnection
{
   ...
   /** The log file (if logging is turned on) - Note that trace data is actually output to the file
       only if the log level of this class is set to DEBUG */
   private LoggerIf m_logger;

   /** The Ganymed SSH connection */
   private Connection m_nexus;
   ...
   SshConnection(Properties connectionProperties, final HelloResponseProcessorIf hrp, boolean connectNow) throws RuntimeException
   {
      ...
      m_logger = (LoggerIf) connectionProperties.get("netconfTraceFile");
   ...
   public void connect()
   {
        ...
        try
        {
            ...
            m_inputStream = new BufferedInputStream(m_session.getStdout()); //new BufferedInputStream(new StreamGobbler(m_session.getStdout()));
            m_outputStream =  new BufferedOutputStream((m_session.getStdin()));
        }
        catch (final Exception ex)
        {
   ...
   {
      if (m_logger != null && s_logger.isDebugEnabled())
      {
         StringBuilder buf = new StringBuilder("");
